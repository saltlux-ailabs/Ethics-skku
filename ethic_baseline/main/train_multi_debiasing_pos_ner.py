...
def preprocess_function(examples):
    ...
    for i, data_i in enumerate(zip(examples['NER_results'], examples['POS_results'])):
        for j, (n_i, p_i) in enumerate(zip(data_i[0], data_i[1])):
            if n_i != "O" and p_i != "O":
            # if p_i != "O":
                entity_mask[i][j] = 1
...
if args.find_hyper:
    ...
    with open('./fine_hyper_pos_ner_union.json', 'r') as f:
        logs = json.load(f)

    logs[name] = {'best_epoch': best_epoch, 'best_f1': best_f1}

    with open('./fine_hyper_pos_ner_union.json', 'w') as f:
